...
import           Control.Monad (foldM, MonadPlus(..), msum, forM, guard)
    ...
    go p_inner = muxes p_inner $ \p -> fmap getConst $ W4B.idxCacheEval cache p $ do
      ...
      case p of
        ...
        W4B.NonceAppExpr a0 -> case W4B.nonceExprApp a0 of
          W4B.FnApp fn (Ctx.Empty Ctx.:> arg1 Ctx.:> arg2) | W4.BaseBoolRepr <- W4.exprType p -> 
            Const <$> binOp (liftPred (\sym' e1' e2' -> W4.applySymFn sym' fn (Ctx.empty Ctx.:> e1' Ctx.:> e2'))) arg1 arg2
          _ -> TFC.foldrMFC acc mempty (W4B.nonceExprApp a0)
        _ -> return $ mempty
...
mkSymbol :: String -> W4.SolverSymbol
mkSymbol s = W4.safeSymbol $ concat $ map go s
  where
    go :: Char -> String
    go = \case
      '→' -> "_arrow_"
