...
data TotalityResult sym arch
  ...
  | TotalityCheckCounterexample (ObservableCounterexample sym arch) (TotalityCounterexample (MM.ArchAddrWidth arch))
  ...
  withSym $ \sym ->
    do
       ...
       goalSat "doCheckTotality" asm $ \res -> case res of
         ...
         Sat evalFn' -> do
           ocex <- getCounterExample' evalFn' bundle
           withGroundEvalFn evalFn' $ \evalFn -> do
      ...
      a <- case mchoice of
        ...
        _ -> do
          () <- withTracing @"message" "Equivalence Counter-example" $ withSym $ \sym -> do
            ...
            res <- getCounterExample bundle (W4.falsePred sym)
